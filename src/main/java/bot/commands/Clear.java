...
public class Clear implements CommandExecutor {
    ...
    public void run(Message message) {
        ...
        if (args.length < 2) {
            Bot.sendExpireMessage(channel, Messages.ERROR_TOO_FEW_ARGUMENTS.message(), 10000);
            return;
        }
        ...
        try {
            ...
            Bot.sendExpireMessage(channel, "Valor `amount:` inválido. Por favor forneça um numero entre `1 e 100`.", 10000);
            return;
        }

        if (amount < 1 || amount > 100) {
            Bot.sendExpireMessage(channel, "A quantidade de mensagens a serem apagadas deve estar entre `1 e 100`.\n*Será possível até __1000__ em breve (ou quando o Leo13 parar de procrastinar e adicionar essa função extra em mim)*", 10000);
        ...
        Bot.sendExpireMessage(channel, "Prontinho, `%s %s` foram de vapo 👍", 5000, amountStr, suffix);
    }
}